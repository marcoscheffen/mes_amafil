...
async function check() {
  ...
  const { data: comps } = await s.rpc('get_user_companies');
  const compId = comps[0]?.company_id;
  
  const { data: msg } = await s.from('chat_message').select('id, phone, user_id').eq('id', 'bc0c511b-f4e4-4e35-aa66-95d3c535aafb');
  console.log("Message:", msg);
  
  if (msg && msg.length > 0) {
      const p = msg[0].phone;
      const { data: clients } = await s.from('clients').select('id, phone, company_id').eq('phone', p);
